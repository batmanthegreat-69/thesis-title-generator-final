...
// @xenova/transformers is an ES Module. This project is CommonJS (require-
// based), and Node — especially Vercel's stricter runtime — won't let a
// CommonJS file require() an ESM package directly. A dynamic import() works
// from CommonJS, so we lazy-load it that way instead.
let transformersModule = null;
async function getTransformers() {
  if (!transformersModule) {
    transformersModule = await import('@xenova/transformers');

    // Vercel's filesystem is read-only except /tmp, so the model cache has to
    // go there when running on Vercel. Locally (npm start), leave the
    // library's default cache location alone.
    if (process.env.VERCEL) {
      transformersModule.env.cacheDir = '/tmp/xenova-cache';
    }
  }
